...
\subsubsection*{2.\ Majority Vote (Classification Rule)}
...
\subsubsection*{3.\ Class Frequency Ratio (Probability Estimation)}
...
The notebook's final summary reports the best configuration for each feature representation as follows:

\vspace{4pt}
\begin{center}
\begin{tabular}{lccc}
    oprule
    extbf{Method} & \textbf{Best $K$} & \textbf{Test Acc.} & \textbf{Test F1} \\
\midrule
    extbf{Flatten} & \textbf{3} & \textbf{0.9959} & \textbf{0.9959} \\
HOG              & 11         & 0.9953          & 0.9953          \\
PCA (95\%)       & 1          & 0.9929          & 0.9929          \\
\bottomrule
\end{tabular}
\end{center}
\vspace{4pt}
...
\[
\begin{pmatrix} 1396 & 6 \\ 6 & 1548 \end{pmatrix}
\]

The best overall configuration is \textbf{Flatten with $K=3$}, with 99.59\% accuracy and an F1-score of 0.9959. Digits 0 and 1 are geometrically distinct, making raw pixel distances sufficient for strong separation without dimensionality reduction.
